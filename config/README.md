...
**Переменные окружения процесса** (в т.ч. `sudo env REF=main make start`) **выше**, чем значения из `config/*.env`. Это нужно, чтобы одноразово переопределять параметры без правки файлов.
...
Логическое окружение Ansible (`ctl` / `stage` / `prod`) для плейбуков задаётся **в приватном репозитории** в файле **`config/ansible-vars.yml`** (ключ **`env`**) после **`make init`** в клоне приватного репо — не в `config/host.env` публичного репо.
...
Значение **`REF`** из командной строки имеет приоритет над **`config/repos.env`**.
