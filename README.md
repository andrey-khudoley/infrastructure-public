...
Запуск самих фаз `stage1` → `stage2` → `runtime` — это **отдельные ручные шаги** на стороне приватного репозитория (`make stage1`, затем `make stage2`, затем при необходимости `make runtime`). Никаких systemd-юнитов и автоматических переходов между фазами bootstrap **не используется**: после `make install-deps` шаг **90** при отсутствии файла создаёт **`config/ansible-vars.yml`** из шаблона и вызывает интерактивное **`make update-sysuser`** (имя и пароль администратора в этом YAML), затем `make start` печатает подсказку: **`make init`** и **`make stage1`** в клоне.

## Роль репозиториев

| Репозиторий | Роль |
|-------------|------|
| **Этот (public)** | Подготовка хоста: диски и swap (**20**), пакеты (**30**); шаг **40** при необходимости приводит **`REPO_URL`** с **`github.com`** по HTTPS к **`git@…`** и готовит deploy key; иначе задайте SSH-URL сами; клон в `PULL_DIR`, **`make install-deps`** в каталоге клона; шаг **90** — повторный `distro-sync`, проверки сервисов, при необходимости **`config/ansible-vars.yml`** из шаблона, **`make update-sysuser`** в клоне; затем подсказка: **`make init`**, **`make stage1`**. |
...
Публичный сценарий **не** дублирует содержимое приватного **`Makefile`**: он гарантирует окружение, вызывает **`make install-deps`** с согласованным набором переменных (см. раздел **«Контракт: переменные для `make install-deps`»**), а на шаге **90** — подготовка **`ansible-vars.yml`** при отсутствии и **`make update-sysuser`**. Запуск **`make init`** и **`make stage1`** — за пользователем.
...
4. После завершения `make start` (включая интерактивное **`make update-sysuser`** на шаге **90**) напечатает подсказку — вручную выполнить `cd $PULL_DIR && sudo make init` (если ещё не делали) и `sudo make stage1`.
...
Минимальный пример с **дефолтами из `config/*.env`** после **`make init`** (ветка `main`; **`REPO_URL`** по умолчанию — HTTPS на GitHub, в шаге **40** он приводится к **`git@github.com:…`**, затем выводится deploy key — добавьте его в репозиторий на GitHub). Логическое окружение Ansible (`ctl` / `stage` / `prod`) задаётся в приватном репо в **`config/ansible-vars.yml`** (ключ **`env`**) после **`make init`** в клоне:

```bash
git clone https://github.com/andrey-khudoley/infrastructure-public.git
cd infrastructure-public
make init
sudo env REF=main make start
# по окончании:
cd /var/lib/infra/src        # PULL_DIR из config/host.env
sudo make init                 # при первом входе: шаблоны → боевой config/ (в т.ч. ansible-vars.yml)
sudo make stage1             # после успеха — подсказка про stage2 (или ребут)
sudo make stage2             # после успеха — подсказка про runtime
sudo make runtime            # по необходимости; логическое окружение — ключ env в config/ansible-vars.yml
```
...
```bash
make init
sudo env REF=main REPO_URL=git@github.com:andrey-khudoley/infrastructure-private.git make start
```
...
Цели `make` задают `ROOT="$(CURDIR)"` для `run-*`; сами скрипты используют `ROOT`, если он уже в окружении, иначе вычисляют корень как родитель каталога `scripts/` (должно совпадать с корнем клона). Переменные окружения (`REF`, `REPO_URL`, `PULL_DIR`, `PUBLIC_*`, `INFRA_SSH_*`, `GALAXY_*`) наследуются при запуске `sudo env ... make <target>`.
...
`install-deps` сначала создаёт `.venv` и ставит из `config/constraints.txt` закреплённый `ansible-core` и Python-зависимости (`scripts/install-python-deps.sh`), затем устанавливает коллекции Galaxy (офлайн-кэш + `--offline`) в repo-local путь `collections/ansible_collections`. На шаге **90** (`scripts/90-finalize.sh`) выполняются повторный `distro-sync`, проверки **sshd**/при необходимости **NetworkManager**, при отсутствии файла — **`config/ansible-vars.yml`** из **`ansible-vars.yml.example`**, затем интерактивное **`make update-sysuser`** в корне клона (имя и пароль системного администратора в `config/ansible-vars.yml`). **Сам `ansible-playbook` bootstrap не запускает** — далее пользователь выполняет **`make init`** и фазы `stage1` / `stage2` / `runtime` вручную в каталоге клона по подсказке.
...
```bash
REF=v1.2.0 REPO_URL=git@github.com:andrey-khudoley/infrastructure-private.git make start
```

Ветка по умолчанию — `main` (задаётся в **`config/repos.env`** или через **`REF=…`**):
...
```bash
PULL_DIR=/opt/infra/src REF=main REPO_URL=git@github.com:andrey-khudoley/infrastructure-private.git make start
```
...
```bash
SKIP_ANSIBLE=1 REPO_URL=git@github.com:andrey-khudoley/infrastructure-private.git make start
```
...
```bash
REPO_URL=git@github.com:andrey-khudoley/infrastructure-private.git make start
```
...
```bash
INFRA_SSH_SKIP_PROMPT=1 REPO_URL=git@github.com:andrey-khudoley/infrastructure-private.git make start
```
...
```bash
MAIN_DISK_DEVICE=/dev/sda REPO_URL=git@github.com:andrey-khudoley/infrastructure-private.git make start
```
...
sudo env REF=main make update
```

`make update` выполняет sync публичного и **приватного** клонов (`PULL_DIR` из **`config/host.env`** или окружения), без установки каких-либо systemd-юнитов. В конце напечатает подсказку: в корне **приватного** клона вручную запустить `sudo make runtime` (и при необходимости снова `make stage1` / `make stage2`).

Точечные git-режимы:

```bash
sudo env REF=main make git-public   # только public sync
sudo env REF=main make git-private  # только private sync
```
...
│   ├── lib/
│   │   ├── load-env.sh      # читает config/*.env (в т.ч. disk.env); env побеждает config; REF_VALUE / PUBLIC_REF_VALUE
...
│   └── 90-finalize.sh       # distro-sync, проверки, ansible-vars.yml при отсутствии, update-sysuser, подсказка make init + stage1
```
...
| Было в `.env` | Куда |
|---------------|------|
| `PULL_DIR`, `SKIP_ANSIBLE` | `config/host.env` |
...
|------|------|------------|
| [config/host.env.example](config/host.env.example) | `config/host.env` | `PULL_DIR`, `SKIP_ANSIBLE` |
...
Читает файлы **`config/host.env`**, **`config/repos.env`**, **`config/ssh.env`**, **`config/galaxy.env`**, **`config/disk.env`** по порядку; отсутствующий файл пропускается. Для каждого ключа выставляет значение только если переменная **ещё не задана** в окружении процесса (**переменные окружения побеждают** `config/`). Затем задаёт **`REF_VALUE`** из **`REF`** (запасное значение `main`) и **`PUBLIC_REF_VALUE`** из **`PUBLIC_REF`**. Преобразований URL здесь нет: приведение **`https://github.com/…`** к **`git@…`** выполняется в шаге **40** (`normalize_github_https_repo_url` в **`scripts/lib/common.sh`**). Пояснения к переменным — в **`config/*.env`** и в номерных скриптах **`scripts/NN-*.sh`**.
...
Запуск **самих фаз** Ansible (`stage1`, `stage2`, `runtime`) на этом шаге **не делается**: пользователь запускает их вручную через `make stage1`/`make stage2`/`make runtime` в каталоге клона. В окружение `make install-deps` передаются **`REPO_URL`**, **`REF`**, **`PULL_DIR`**, а также **`GALAXY_*`** и при необходимости **`COLLECTIONS_REQ`** (по умолчанию `PULL_DIR/collections/requirements.yml`).
...
3. **`run_update_sysuser`** — в каталоге клона **`PULL_DIR`**: если **`config/ansible-vars.yml`** отсутствует — копия из **`ansible-vars.yml.example`**; затем **`make update-sysuser`** (интерактивно: имя и пароль администратора в этом YAML). Полный **`make init`** здесь не вызывается. Требуется **`Makefile`** с целью **`update-sysuser`**.
4. **`print_next_step_hint`** — итоговая подсказка с готовой командой `cd ${PULL_DIR} && sudo make init && sudo make stage1` (с учётом `SKIP_ANSIBLE`). Дальше пользователь сам запускает фазы Ansible.

## Связь с приватным репозиторием

Точка входа на стороне приватного репозитория — **`Makefile`** с целями **`install-deps`** (Ansible Galaxy + venv), **`update-sysuser`** (интерактивная правка `config/ansible-vars.yml`; вызывается из шага **90** после `install-deps`), **`stage1`** / **`stage2`** / **`runtime`** (фазы единого `playbooks/site.yml`). Публичный bootstrap после клона в **`PULL_DIR`** выполняет **`make install-deps`**, затем на шаге **90** — **`make update-sysuser`**, передавая в окружение **`REPO_URL`**, **`REF`**, **`PULL_DIR`** (как и для `install-deps`). Логическое окружение узла (`ctl` / `stage` / `prod` и прочие ключи для ролей) задаётся **только** в приватном **`config/ansible-vars.yml`** (поле **`env`** и остальные переменные плейбука). Сами фазы Ansible запускает пользователь:

```bash
sudo make init             # при необходимости: шаблоны → боевой config/
sudo make stage1           # подскажет команду для stage2; при необходимости перезагрузит сервер
sudo make stage2           # подскажет команду для runtime
sudo make runtime          # по необходимости (никаких таймеров и юнитов больше нет)
```
...
3. Сохраняйте согласованность имён: в публичном bootstrap внешний интерфейс для ветки — **`REF`**; внутри shell после **`load-env.sh`** используется **`REF_VALUE`**.
